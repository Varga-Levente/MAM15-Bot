...
const unsigned long LORA_HEALTH_CHECK_INTERVAL = 5000; // 5 másodpercenként egyszer
...
const unsigned long LORA_RECONNECT_TIMEOUT = 10000; // 10 másodperc maximum újracsatlakozásra

// ===== CRC számoló objektum =====
CRC16 crcCalculator(CRC_POLYNOMIAL, CRC_INITIAL_VALUE, CRC_FINAL_XOR_VALUE, true, true);

/**
 * LoRa modul újraindítása
 */
bool restartLoRa() {
...
/**
 * LoRa modul állapotellenőrzése
 */
void checkLoRaHealth() {
  unsigned long currentTime = millis();
  
  // Csak bizonyos időközönként végezzük az ellenőrzést
  if (currentTime - lastLoRaHealthCheck < LORA_HEALTH_CHECK_INTERVAL) {
    return;
  }
  lastLoRaHealthCheck = currentTime;
  
  // Ellenőrizzük a LoRa modul állapotát
  bool loraWorking = false;
  
  // Egyszerű ellenőrzés: megpróbálunk egy csomagot elemezni (nem blokkoló)
  int packetSize = LoRa.parsePacket();
  if (packetSize > 0) {
    loraWorking = true;
  }
  
  // Ha a LoRa nem működik, de korábban működött
  ...
  // Újracsatlakozási folyamat kezelése
  ...
  // Ha túl sokszor újraindítottuk, álljunk le
  if (loraRestartCount >= MAX_LORA_RESTARTS) {
    ...
    // Itt lehetne további műveletek, pl. LED villogtatás
  }
}
...
void setup() {
  ...
  // Reset pin beállítása kimenetként
...
void loop() {
  ...
  // Csak akkor próbáljunk csomagot fogadni, ha a LoRa modul egészséges
