...
String codesPage();
esp_err_t handleExternalTrigger(httpd_req_t *req);
...
void handleSettings(httpd_req_t *req){
  ...
  if(len > 0){
    ...
    if(pauseIdx >= 0){
      int ampPos = arg.indexOf('&', pauseIdx);
      String pauseStr = (ampPos > 0) ? arg.substring(pauseIdx+6, ampPos) : arg.substring(pauseIdx+6);
    ...
    if(extIdx >= 0){
      ...
      Serial.printf("🔌 Külső trigger: %s\n", externalTriggerEnabled ? "ENGEDÉLYEZVE" : "LETILTVA");
    }
...
esp_err_t handleExternalTrigger(httpd_req_t *req){
  ...
  if(len > 0){
    ...
    int modeIdx = arg.indexOf("extmode=");
    if(modeIdx >= 0){
      String modeStr = arg.substring(modeIdx+8);
